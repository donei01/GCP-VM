...
with open('large.csv','r') as csvinput:
	with open('large_extended.csv','w') as csvoutput:
		...
		row.extend(['RELATED_ACCT','RELATED_TYPE','PARTY_TYPE','PARTY_RELATION','PARTY_STARTDATE','PARTY_ENDDATE','LARGE_CASH_EXEMPT',\
				'DEMARKET_FLAG','DEMARKET_DATE','PROB_DEFAULT_RISKR','OFFICIAL_LANG_PREF','CONSENT_SHARING','PREFERRED_CHANNEL','PRIMARY_BRANCH_NO',\		
				'CUSTOMER_STATUS','DEPENDANTS_COUNT','SEG_MODEL_ID','SEG_MODEL_TYPE','SEG_MODEL_NAME','SEG_MODEL_GROUP','SEG_M_GRP_DESC',\
				'SEG_MODEL_SCORE','ARMS_MANUFACTURER','AUCTION','CASHINTENSIVE_BUSINESS','CASINO_GAMBLING','CHANNEL_ONBOARDING',\
				'CHANNEL_ONGOING_TRANSACTIONS',\
				'CLIENT_NET_WORTH','COMPLEX_HI_VEHICLE','DEALER_PRECIOUS_METAL','DIGITAL_PM_OPERATOR','EMBASSY_CONSULATE',\
				'EXCHANGE_CURRENCY','FOREIGN_FINANCIAL_INSTITUTION',\
				'FOREIGN_GOVERNMENT','FOREIGN_NONBANK_FINANCIAL_INSTITUTION','INTERNET_GAMBLING','MEDICAL_MARIJUANA_DISPENSARY',\
				'MONEY_SERVICE_BUSINESS','NAICS_CODE',\
				'NONREGULATED_FINANCIAL_INSTITUTION','NOT_PROFIT','OCCUPATION','PRIVATELY_ATM_OPERATOR','PRODUCTS','SALES_USED_VEHICLES',\
				'SERVICES','SIC_CODE',\
				'STOCK_MARKET_LISTING','THIRD_PARTY_PAYMENT_PROCESSOR','TRANSACTING_PROVIDER','ADDITIONAL_NAME','HIGH_NET_WORTH'])
		...
		for i in range(16800000):   
			...
			if HighNetWorthFlag = 'Yes': 
				...
				max(max((randrange(0,100,1)-99),0)* randrange(40000,50000,1),randrange(-250000,600000,1),max((randrange(0,100,1)-99),0)* randrange(145000,15500,1))
